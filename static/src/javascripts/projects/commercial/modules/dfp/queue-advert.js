...
function getAdBreakpointSizes(advert) {
    return detect.breakpoints.reduce((sizes, breakpoint) => {
        const data = advert.node.getAttribute(`data-${breakpointNameToAttribute(breakpoint.name)}`);
        if (data) {
...
function createSizeMapping(attr) {
    return attr.split('|').map(size => size === 'fluid' ? 'fluid' : size.split(',').map(Number));
}
