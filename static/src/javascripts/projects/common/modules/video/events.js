...
function bindGoogleAnalyticsEvents(player, canonicalUrl) {
    ...
    Object.keys(events).map(eventName => `media:${eventName}`).forEach((playerEvent) => {
...
function ophanRecord(id, event, player) {
    const ophanPath = isEmbed ? 'ophan/embed' : 'ophan/ng';
    if (id) {
        require([ophanPath], (ophan) => {
            const eventObject = {};
...
function initOphanTracking(player, mediaId) {
    EVENTS.concat(QUARTILES.map(q => `content:${q}`)).forEach((event) => {
        player.one(constructEventName(event, player), (event) => {
...
function kruxTracking(player, event) {
    const desiredVideos = ['gu-video-457263940', 'gu-video-55e4835ae4b00856194f85c2'];
...
function bindContentEvents(player) {
    const events = {
        ...
        timeupdate() {
            const progress = Math.round(parseInt(player.currentTime() / player.duration() * 100, 10));
